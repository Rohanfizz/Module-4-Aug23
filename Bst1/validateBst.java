...
class Solution{
	...
	Pair validate(Node node){
		...
		boolean amIBst = left.isBst && right.isBst && (left.maxi < node.data)
			&& (right.mini > node.data);
		if(!amIBst) return new Pair(false,0,0);
		
		int myMax = Math.max(node.data,right.maxi);
		int myMin = Math.min(node.data,left.mini);
